...
lazy_static! {
    static ref CALLBACK_SENDERS_GET_SECRETS: Arc<Mutex<HashMap<i32, oneshot::Sender<Result<Option<String>>>>>> = Arc::new(Mutex::new(HashMap::new()));
    static ref CALLBACK_SENDERS_FIND_SECRETS: Arc<Mutex<HashMap<i32, oneshot::Sender<Result<Vec<String>>>>>> = Arc::new(Mutex::new(HashMap::new()));
}
...
impl OnGetSecretResult for OnGetSecretResultAdapter {
    fn success(&self, result: Option<String>) {
        CALLBACK_SENDERS_GET_SECRETS.lock().unwrap().remove(&self.cb_id).unwrap().send(
            Ok(result)
        ).unwrap();
    }
...
impl OnFindSecretsResult for OnFindSecretsResultAdapter {
    fn success(&self, result: Vec<String>) {
        CALLBACK_SENDERS_FIND_SECRETS.lock().unwrap().remove(&self.cb_id).unwrap().send(
            Ok(result)
        ).unwrap();
    }
