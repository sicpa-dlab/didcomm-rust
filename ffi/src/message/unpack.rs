use didcomm::{Message, UnpackMetadata, UnpackOptions, error::ErrorKind};

use crate::common::EXECUTOR;
use crate::{ErrorCode, FFIDIDResolver, FFISecretsResolver, did_resolver_adapter::FFIDIDResolverAdapter, secrets_resolver_adapter::FFISecretsResolverAdapter};
...
pub fn unpack<'a>(
    ...
    let future = async move {
        Message::unpack(&msg, &did_resolver, &secret_resolver, &options)
            .await
    };
...
mod tests {
    ...
    use crate::test_vectors::{ALICE_DID, ALICE_DID_DOC, BOB_DID, BOB_DID_DOC, ALICE_SECRETS, BOB_SECRETS};
    ...
    async fn test_unpack_encrypted_works() {
        ...
        let did_resolver =
            Box::new(ExampleFFIDIDResolver::new(vec![ALICE_DID_DOC.clone(), BOB_DID_DOC.clone()]));
        let secrets_resolver = 
            Box::new(ExampleFFISecretsResolver::new( BOB_SECRETS.clone()));              
        let unpack_cb = create_unpack_cb();
        let unpack_cb_cb_id = unpack_cb.cb_id;       
        unpack(
            res,
