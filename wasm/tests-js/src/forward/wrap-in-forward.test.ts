...
import {
  ...
  test.each([
    ...
    async ({ message, headers, to, routing_keys, enc_alg_anon, did_resolver }) => {
      const res = await Message.wrap_in_forward(JSON.stringify(message), headers, to, routing_keys, enc_alg_anon, did_resolver);
