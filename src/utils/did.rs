    ...
    fn as_key_pair(&self) -> Result<KnownKeyPair> {
        match (&self.type_, &self.verification_material) {
            ...
            (VerificationMethodType::X25519KeyAgreementKey2019, VerificationMaterial::Base58(ref b58_value)) => {
                let decoded_value = b58_value.from_base58().kind(ErrorKind::IllegalArgument, "Wrong base58 value in verification material")?;
                let base64_url_value = base64::encode_config(&decoded_value, base64::URL_SAFE_NO_PAD);
                let jwk = json!({"kty": "OKP",
                    "crv": "X25519",
                    "x": base64_url_value});

                X25519KeyPair::from_jwk_value(&jwk)
                    .kind(ErrorKind::Malformed, "Unable parse base58 verification material")
                    .map(KnownKeyPair::X25519)
            }

            (VerificationMethodType::Ed25519VerificationKey2018, VerificationMaterial::Base58(ref b58_value)) => {
                let decoded_value = b58_value.from_base58().map_err(|e| err_msg(ErrorKind::IllegalArgument, "Wrong base58 value in verification material"))?;
                let base64_url_value = base64::encode_config(&decoded_value, base64::URL_SAFE_NO_PAD);
                let jwk = json!({"kty": "OKP",
                    "crv": "Ed25519",
                    "x": base64_url_value});
            ...
            (VerificationMethodType::X25519KeyAgreementKey2020, VerificationMaterial::Multibase(ref multibase_value)) => {
                if !multibase_value.starts_with('z') {
                    Err(err_msg(ErrorKind::IllegalArgument, "Multibase must start with 'z'"))?
                };
                let decoded_value = multibase_value.split_at(1).1.from_base58().map_err(|e| err_msg(ErrorKind::IllegalArgument, "Wrong multibase value in verification material"))?;
                let (codec, decoded_value) = from_multicodec(&decoded_value)?;
                if codec != Codec::X25519Pub {
                    Err(err_msg(ErrorKind::IllegalArgument, "Wrong codec in multibase secret material"))?
                }
                let base64_url_value = base64::encode_config(&decoded_value, base64::URL_SAFE_NO_PAD);
                ...
                X25519KeyPair::from_jwk_value(&jwk)
                    .kind(ErrorKind::Malformed, "Unable parse multibase verification material")
                    .map(KnownKeyPair::X25519)
            }
    ...
    fn as_key_pair(&self) -> Result<KnownKeyPair> {
        match (&self.type_, &self.secret_material) {
            ...
            (SecretType::X25519KeyAgreementKey2019, SecretMaterial::Base58(ref b58_value)) => {
                let decoded_value = b58_value.from_base58().map_err(|e| err_msg(ErrorKind::IllegalArgument, "Wrong base58 value in secret material"))?;
            ...
            (SecretType::X25519KeyAgreementKey2020, SecretMaterial::Multibase(ref multibase_value)) => {
                ...
                let decoded_b58_value = multibase_value.split_at(1).1.from_base58().map_err(|e| err_msg(ErrorKind::IllegalArgument, "Wrong base58 value in secret material"))?;
                let (codec, decoded_value) = from_multicodec(&decoded_b58_value)?;
                ...
                X25519KeyPair::from_jwk_value(&jwk)
                    .kind(ErrorKind::Malformed, "Unable parse multibase secret material")
                    .map(KnownKeyPair::X25519)
            }
...
impl Codec {
    fn value(&self) -> u32 {
        match *self {
            Codec::X25519Pub => 0xEC,
            Codec::Ed25519Pub => 0xED,
            Codec::Ed25519Priv => 0x1300,
            Codec::X25519Priv => 0x1302,
        }
    }

    fn get_codec_by_prefix(value: &u32) -> Result<Codec> {
        return match value {
...
pub(crate) fn from_multicodec(value: &Vec<u8>) -> Result<(Codec, &[u8])> {
    ...
    let prefix_int = val.read_unsigned_varint_32().map_err(|e| err_msg(ErrorKind::IllegalArgument, "Wrong prefix in verification material"))?;
    let codec = Codec::get_codec_by_prefix(&prefix_int)?;
    let mut prefix: Cursor<Vec<u8>> = Cursor::new(Vec::new());
    prefix.write_unsigned_varint_32(prefix_int).map_err(|e| err_msg(ErrorKind::IllegalArgument, "Wrong prefix in verification material"))?;
...
mod tests {
    ...
    fn secret_as_key_pair_x25519_2019_base58_works() {
        ...
        })).map(KnownKeyPair::X25519).unwrap();
    ...
    fn secret_as_key_pair_ed25519_2018_base58_works() {
        ...
        let expected_key = Ed25519KeyPair::from_jwk_value(&json!({
            ...
            "d": "T2azVap7CYD_kB8ilbnFYqwwYb5N-GcD6yjGEvquZXg"
        })).map(KnownKeyPair::Ed25519).unwrap();
        assert_eq!(format!("{:?}", actual_key), format!("{:?}", expected_key));
    }
    ...
    fn verification_method_as_key_pair_x25519_2019_base58_works() {
        let actual_key = VerificationMethod {
            ...
            verification_material: (VerificationMaterial::Base58("JhNWeSVLMYccCk7iopQW4guaSJTojqpMEELgSLhKwRr".to_string()))
        }.as_key_pair().unwrap();
    ...
    fn verification_method_as_key_pair_ed25519_2018_base58_works() {
        let actual_key = VerificationMethod {
            ...
            verification_material: (VerificationMaterial::Base58("ByHnpUCFb1vAfh9CFZ8ZkmUZguURW8nSw889hy6rD8L7".to_string()))
        }.as_key_pair().unwrap();
    ...
    fn verification_method_as_key_pair_x25519_2020_multibase_works() {
        let actual_key = VerificationMethod {
            ...
            controller: "did:example:eve#key-x25519-1".to_string(),
            verification_material: (VerificationMaterial::Multibase("z6LSbysY2xFMRpGMhb7tFTLMpeuPRaqaWM1yECx2AtzE3KCc".to_string()))
        }.as_key_pair().unwrap();
    ...
    fn verification_method_as_key_pair_ed25519_2020_multibase_works() {
        let actual_key = VerificationMethod {
            ...
            controller: "did:example:eve#key-ed25519-1".to_string(),
            verification_material: (VerificationMaterial::Multibase("z6MkqRYqQiSgvZQdnBytw86Qbs2ZWUkGv22od935YF4s8M7V".to_string()))
        }.as_key_pair().unwrap();
